...
Content-aware keyframe sampler with adaptive optical-flow-based motion scoring.
...
Recommended first run:
    python .\content_aware_sampler_adaptive_flow.py `
...
def compute_clip_dynamic_score(candidates: List[CandidateFrame]) -> float:
    """
    Estimate the dynamic level of the whole clip.

    This clip-level score controls how strongly optical flow should affect
    keyframe selection. It is not passed to the model directly.
    """
    if not candidates:
        return 0.0

    flow_scores = np.array([c.flow_score_raw for c in candidates], dtype=np.float32)
    motion_areas = np.array([c.flow_motion_area_raw for c in candidates], dtype=np.float32)
    frame_diffs = np.array([c.frame_diff_raw for c in candidates], dtype=np.float32)
    hist_diffs = np.array([c.hist_diff_raw for c in candidates], dtype=np.float32)

    mean_flow = float(np.mean(flow_scores))
    p75_flow = float(np.percentile(flow_scores, 75))
    mean_motion_area = float(np.mean(motion_areas))

    frame_diff_component = float(np.clip(np.mean(frame_diffs) / 35.0, 0.0, 1.0))
    hist_diff_component = float(np.clip(np.mean(hist_diffs) / 0.50, 0.0, 1.0))

    dynamic_score = (
        0.35 * p75_flow
        + 0.25 * mean_flow
        + 0.20 * mean_motion_area
        + 0.10 * frame_diff_component
        + 0.10 * hist_diff_component
    )

    return float(np.clip(dynamic_score, 0.0, 1.0))


def choose_adaptive_flow_weight(
    clip_dynamic_score: float,
    static_flow_weight: float,
    medium_flow_weight: float,
    dynamic_flow_weight: float,
    dynamic_score_low: float,
    dynamic_score_high: float,
) -> Tuple[float, str]:
    """
    Choose optical flow weight based on the clip-level dynamic score.
    """
    if clip_dynamic_score < dynamic_score_low:
        return static_flow_weight, "static_or_dialogue_heavy"

    if clip_dynamic_score < dynamic_score_high:
        return medium_flow_weight, "moderately_dynamic"

    return dynamic_flow_weight, "motion_heavy"
...
def process_video(video_path: Path, output_root: Path, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    clip_dynamic_score = compute_clip_dynamic_score(candidates) if args.use_optical_flow else 0.0

    if args.use_optical_flow and args.adaptive_flow_weight:
        effective_flow_weight, dynamic_category = choose_adaptive_flow_weight(
            clip_dynamic_score=clip_dynamic_score,
            static_flow_weight=args.static_flow_weight,
            medium_flow_weight=args.medium_flow_weight,
            dynamic_flow_weight=args.dynamic_flow_weight,
            dynamic_score_low=args.dynamic_score_low,
            dynamic_score_high=args.dynamic_score_high,
        )
    else:
        effective_flow_weight = args.flow_weight if args.use_optical_flow else 0.0
        dynamic_category = "fixed_flow_weight" if args.use_optical_flow else "no_optical_flow"
    ...
    make_contact_sheet(
        ...
        title=f"{clip_stem}{args.output_suffix} | content-aware + adaptive optical flow",
        thumb_width=args.contact_sheet_thumb_width,
    )

    metadata_path = clip_output_dir / "selected_frames.json"
    result = {
        "sampler_version": "content_aware_sampler_adaptive_flow_v1",
        ...
        "scoring_config": {
            ...
            "ideal_brightness": args.ideal_brightness,
            "use_optical_flow": args.use_optical_flow,
            "adaptive_flow_weight": args.adaptive_flow_weight,
            "clip_dynamic_score": clip_dynamic_score,
            "dynamic_category": dynamic_category,
            "effective_flow_weight": effective_flow_weight,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-suffix", default="_adaptive_flow", help="Suffix added to each per-clip output folder, e.g. ep02_clip_0000_flow.")
    parser.add_argument("--summary-name", default="sampling_summary_adaptive_flow.jsonl", help="Summary JSONL filename.")
    ...
    parser.add_argument("--flow-weight", type=float, default=0.25, help="Fixed optical flow weight. Used only when --adaptive-flow-weight is not enabled.")
    parser.add_argument("--adaptive-flow-weight", action="store_true", help="Automatically adjust optical flow weight according to clip-level dynamic score.")
    parser.add_argument("--static-flow-weight", type=float, default=0.05, help="Flow weight for static/dialogue-heavy clips.")
    parser.add_argument("--medium-flow-weight", type=float, default=0.15, help="Flow weight for moderately dynamic clips.")
    parser.add_argument("--dynamic-flow-weight", type=float, default=0.25, help="Flow weight for motion-heavy clips.")
    parser.add_argument("--dynamic-score-low", type=float, default=0.20, help="Below this clip_dynamic_score, use static-flow-weight.")
    parser.add_argument("--dynamic-score-high", type=float, default=0.45, help="Above this clip_dynamic_score, use dynamic-flow-weight.")
...
def main() -> None:
    ...
    print("Adaptive flow weight:", args.adaptive_flow_weight)
    if args.use_optical_flow and args.adaptive_flow_weight:
        print("Flow weights:", {
            "static": args.static_flow_weight,
            "medium": args.medium_flow_weight,
            "dynamic": args.dynamic_flow_weight,
            "low_threshold": args.dynamic_score_low,
            "high_threshold": args.dynamic_score_high,
        })
    else:
    ...
    with summary_path.open("w", encoding="utf-8") as summary_file:
        for idx, video_path in enumerate(videos, start=1):
            ...
            try:
                ...
                print("Dynamic score:", cfg.get("clip_dynamic_score"))
                print("Dynamic category:", cfg.get("dynamic_category"))
                print("Effective flow weight:", cfg.get("effective_flow_weight"))
