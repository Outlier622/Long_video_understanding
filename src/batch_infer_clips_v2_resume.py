...
def is_success_record(record: dict) -> bool:
    """
    Treat only successfully parsed JSON records as completed.
    Failed/error records will be retried during resume.
    """
    return bool(record.get("json_parse_ok") is True and record.get("parsed_json") is not None)


def load_completed_success_records(output_path: Path) -> dict:
    """
    Load existing JSONL results and keep only the latest successful record for each clip_id.
    This allows safe resume even if the output file contains old error records.
    """
    completed = {}
    ...
    with output_path.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                print(f"Skipping invalid JSONL line {line_no} in existing output.")
                continue
...
def backup_existing_output(output_path: Path) -> Path | None:
    """
    Back up the existing JSONL before rewriting it with deduplicated success records.
    """
    if not output_path.exists():
        return None

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_path = output_path.with_name(output_path.name + f".bak_{timestamp}")
    shutil.copy2(output_path, backup_path)
    return backup_path


def rewrite_success_records(output_path: Path, completed_records: dict) -> None:
    """
    Rewrite OUTPUT_PATH so it only contains completed successful records.
    This removes duplicated old error records and makes the resumed file clean.
    """
    with output_path.open("w", encoding="utf-8") as f:
        for clip_id in sorted(completed_records):
            f.write(json.dumps(completed_records[clip_id], ensure_ascii=False) + "\n")
        f.flush()
...
completed_records = load_completed_success_records(OUTPUT_PATH)
backup_path = backup_existing_output(OUTPUT_PATH)

if backup_path is not None:
    print("Existing output backed up to:", backup_path)

rewrite_success_records(OUTPUT_PATH, completed_records)
completed_ids = set(completed_records.keys())
remaining = [i for i in range(len(clips)) if i not in completed_ids]

print("Resume mode enabled.")
print("Completed successful clips found:", len(completed_ids))
print("Remaining clips to process:", len(remaining))

with OUTPUT_PATH.open("a", encoding="utf-8") as f:
    for i, clip_path in enumerate(clips):
        if i in completed_ids:
            print(f"\nSkipping completed clip {i + 1}/{len(clips)}: {clip_path.name}")
            continue
        ...
        try:
            ...
            record = {
                "version": "v2_domain_accuracy_prompt_fixed_resume",
                "clip_id": i,
