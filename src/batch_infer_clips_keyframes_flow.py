...
MANIFEST_PATH = Path(r"D:\projects\longvideo\episodes\ep02\keyframes\clip_manifest_flow.jsonl")

OUTPUT_TEST_PATH = Path(r"D:\projects\longvideo\episodes\ep02\outputs\ep02_results_keyframes_flow_test.jsonl")
OUTPUT_ALL_PATH = Path(r"D:\projects\longvideo\episodes\ep02\outputs\ep02_results_keyframes_flow.jsonl")

RUN_MODE = "all"

VERSION = "ep02_keyframe_v3_optical_flow_sampling"
...
def load_completed_clip_ids(output_path: Path) -> set:
    ...
    with output_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if clip_id is not None and json_parse_ok is True and error is None:
...
def main() -> None:
    ...
    completed_clip_ids = load_completed_clip_ids(output_path)

    if completed_clip_ids:
        print(f"Already completed clips: {len(completed_clip_ids)}")
        records = [
            r for r in records
            if int(r["clip_id"]) not in completed_clip_ids
        ]

    print(f"Remaining clips to process: {len(records)}")

    if not records:
        print("No remaining records to process. Inference is already complete.")
